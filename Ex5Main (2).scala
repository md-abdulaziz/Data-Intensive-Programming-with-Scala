...
object Ex5Main extends App {
  ...
  val words: Long = articlesRdd.flatMap(_.split(" ")).map((_,1)).count()
  ...
  val chars: Long = articlesRdd.map(value => value.replaceAll(" ","")).flatMap(_.toList).count()

  println(s"#chars = ${chars}")
  var x = articlesRdd.flatMap(line => line.split( "\\D"))
  val numChars: Long = x.map(word => word.toCharArray.length).sum.longValue()
  ...
  val words5Count: Long = articlesRdd.flatMap(line => line.split(" ")).filter(s => !s.endsWith("DisCo") && s.length==5).count()
  ...
  // this will read a text file from local file system and return it as an RDD of string
  // In Scala, flatMap() method is identical to the map() method, but the only difference is that in flatMap the inner grouping of an item is removed and a sequence is generated.

  val lyricsCount = lyricsRdd.flatMap(line => line.split(" ")) // this line will split the line
                             .map(word => (word, 1)) // this line will map word by word 1
                             .reduceByKey((v1, v2) => v1 + v2) // this will reduce the key value after suming them
